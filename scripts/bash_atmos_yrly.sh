...
SIM_STARTMON=${CLIM_STARTMON}
SIM_ENDMON=${CLIM_ENDMON}
SIM_STARTDAY=${CLIM_STARTDAY}
SIM_ENDDAY=${CLIM_ENDDAY}
SIM_STARTHOUR=${CLIM_STARTHOUR}
SIM_ENDHOUR=${CLIM_ENDHOUR}
...
function CHECKCORRECTDAYS()
{
    ...
    #echo "Total days in ${SIM_STARTMON} is ${tday}"
    ...
    endday=$(( SIM_ENDMON + 0 ))  # force convert to integer
    if [[ ${endday} -gt ${tday} ]]
    then
        SIM_ENDDAY=${tday}
    fi
...
{ # Start of code block
    ...
    for y in $(seq ${CLIM_STARTYEAR} ${CLIM_ENDYEAR})
    do
        SIM_STARTYEAR=${y}
        CHECKCORRECTDAYS ${y}
        SIM_ENDYEAR=${y}
        ...
        ttle="#PBS -N ${y}_UAT_${rsltn}-Atmos"
