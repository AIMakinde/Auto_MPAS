...
rundir="cfsr_60km_uniform"			## Name of the model directory to create or over write. Note model excutables, files and output will be copied to this directory
...
LOGFILE="log.cfsr_new_auto_60km"		## This bash script generates a log file. What do you wish to name the logfile?
...
then
	...
	sed -i "3s|.*|${prsd1}|" bash_dwn.sh
	sed -i "4s|.*|${prsd2}|" bash_dwn.sh
	sed -i "5s|.*|${sfcd1}|" bash_dwn.sh
	sed -i "6s|.*|${sfcd2}|" bash_dwn.sh
	sed -i "7s|.*|${sfcd3}|" bash_dwn.sh
...
then
	...
	echo "modifying bash_ung.sh for the current run..."
	sed -i "3s|.*|${bsu1}|" bash_ung.sh
	sed -i "4s|.*|${bsu2}|" bash_ung.sh
	sed -i "5s|.*|${bsu3}|" bash_ung.sh
	sed -i "6s|.*|${bsu4}|" bash_ung.sh
	sed -i "7s|.*|${bsu5}|" bash_ung.sh
	sed -i "8s|.*|${bsu6}|" bash_ung.sh
...
then
	...
	sed -i "23s|.*|${dstr6d}|" run_mpas_atmos.qsub
