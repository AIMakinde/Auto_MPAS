...
for (( i=0; i<$NUM_LEVELS; i++ )); do
    ...
    cdo chname,ta,pa -setconst,${p_value} tmp_template.nc tmp_p_${level_index}.nc
done
...
cdo mergelevels tmp_p_*.nc tmp_p.nc

echo "Merging pressure (p) with T and Td fields..."
...
cdo expr,'r=0.622*es/(pa - es)' tmp_es.nc tmp_r.nc

echo "Calculating potential temperature (theta)..."
cdo expr,'theta=ta*(1000/pa)**0.2857' tmp_all.nc tmp_theta.nc

echo "Merging theta and r fields..."
cdo merge tmp_theta.nc tmp_r.nc tmp_calc.nc

echo "Calculating equivalent potential temperature (theta_e)..."
cdo expr,'thetae=theta*exp(2500000*r/(1004*ta))' tmp_calc.nc theta_e.nc

echo "Equivalent potential temperature calculation complete. Output written to theta_e.nc"

# Clean up temporary files
rm -f tmp_TTd.nc tmp_template.nc tmp_const.nc tmp_p_*.nc tmp_p.nc tmp_all.nc tmp_es.nc tmp_r.nc tmp_theta.nc tmp_calc.nc
