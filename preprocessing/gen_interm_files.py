...
def process_file(args):
    ...
    # upack input arguments
    inputnc, outprefix, outvars, defualtunits = args

    try:
        # Read NetCDF file
        ds = xr.open_dataset(inputnc)

        # # Specifically for ERA5, convert geometry height to geopotential height
        # ds['z'] = ds['z']/9.80665  # standard gravity, g = 9.80665 m/s2
        ...
        for i in range(len(time_str)):
            ...
            if 'plevel_variables' in outvars and outvars['plevel_variables']:
                for gvar, dvar in outvars['plevel_variables'].items():
                    if dvar in ds:
                        data = ds[dvar].values
                        winter_d3 = pyw.V3dp(gvar, data[i, :, :, :], pl_layer, unit=defualtunits[gvar])
            ...
            if 'surface_variables' in outvars and outvars['surface_variables']:
                for gvar, dvar in outvars['surface_variables'].items():
                    if dvar in ds:
                        data = ds[dvar].values
                        winter_d2 = pyw.V2d(gvar, data[i, :, :], unit=defualtunits[gvar])
                    ...
                    if dvar in ds:
                        ...
                        if svar == 'SM':
                            ...
                            if sm_track == 10:  # All soil layers added
                                winter_sl = pyw.Vsl(svar, winter_sl_sm, sl_layer, unit=defualtunits[gvar])
                                total_var.append(winter_sl)
                        elif svar == 'ST':
                            winter_sl_st[j - 1, :, :] = data[i, :, :]
                            st_track += j
                            if st_track == 10:  # All soil layers added
                                winter_sl = pyw.Vsl(svar, winter_sl_st, sl_layer, unit=defualtunits[gvar])
                                total_var.append(winter_sl)
                    else:
...
def main():
    ...
    'SEAICE': 'fraction',     #       fraction Sea-ice fraction
